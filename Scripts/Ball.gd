...
func AnimateHit():
	...
	$Tween.interpolate_property($Sprite, "scale", Vector2(1.2,1.2), Vector2(1,1), .5, Tween.TRANS_SINE, Tween.EASE_OUT)
	$Tween.start()
...
func _on_Ball_body_entered(body):
	if ShouldPlayBounceSound: Global.Game.PlaySample(SampleBounce, 3)
