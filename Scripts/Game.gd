...
func _process(delta):
	if PreViewSize != get_viewport().get_size():
		UpdateScreenSize()
	PreViewSize = get_viewport().get_size()


func _input(ev):
	if ev is InputEventKey and ev.scancode == KEY_C and ev.is_pressed():
		UpdateColor()
...
func Fade(
	...
	if fade_in:
		...
		FadeTween.interpolate_property(
			node.material, "shader_param/burn_position", 1, 0, duration, Trans, Ease
		)
	else:
		node.material.set_shader_param("burn_position", 0)
		FadeTween.interpolate_property(
			node.material, "shader_param/burn_position", 0, 1, duration, Trans, Ease
		)
...
func UpdateColor():
	...
	while Global.isColored == WasColored and not isHighContrast:  # find another color
		...
		NewFBGColor.h += RNG.randf_range(
			FBGColorHueVariance, (HueCoolMax - HueCoolMin) - FBGColorHueVariance
		)
		if NewFBGColor.h > HueCoolMax:
			NewFBGColor.h -= (HueCoolMax - HueCoolMin)
		if NewFBGColor.h < HueCoolMin:
			NewFBGColor.h += (HueCoolMax - HueCoolMin)
		...
		while not isDifferentColor:
			...
			isDifferentColor = (
				abs(PrevBGColor.h - NewBGColor.h) > BGColorHueVariance
				and abs(PrevBGColor.h - NewBGColor.h) < 1 - BGColorHueVariance
			)
...
func Play():
	...
	if ASLTransReset != null and is_instance_valid(ASLTransReset):
		...
		TweenAudioFade.interpolate_property(
			ASLTransReset,
			"volume_db",
			ASLTransReset.volume_db,
			-100,
			.5,
			Tween.TRANS_LINEAR,
			Tween.EASE_IN_OUT
...
func LoadData():
	var DataFile = File.new()
	if not DataFile.file_exists(Global.SavePath):
		return
...
func _on_Ball_hit(data):
	...
	$Tween.interpolate_property(
		Engine, "time_scale", 1.5, Engine.time_scale, .1, Tween.TRANS_SINE, Tween.EASE_OUT
	)
	$Tween.start()
